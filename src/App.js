...
import { isValidPhoneNumber, isBlank } from './helperMethods';
import { sortByAge, sortAlphabetically } from './sortFilters';

// Class that fetches User information from AppSheet's webservice 
...
class App extends Component {
  ...
  render() {
    return (
      <BodyBackgroundColor backgroundColor="#ccffcc">
        <div>
          <h2 class="title">
            Akash Arora - App Sheet Project
          </h2>
          {this.state.hasResults ? (
            <div style={{backgroundColor: '#ffffff'}}>
              <UserTable
                data={this.state.users}
                title="Displaying the Top 5 Youngest Users with Valid Phone Numbers, by Name"
              />
            </div>
          ) : (
  ...
  // post: calls <fetchList> to find User Id's
  handleClick = event => {
    this.fetchList();
  };
  ...
  // post:  Makes a request to AppSheet's web service using optional token to retrieve
  ...
  //       Sorts and Filters Users upon retrieving all users' details in the LAST List of User ID's
